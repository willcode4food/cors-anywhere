...
function onProxyResponse(response, req, res) {
  ...
  if(statusCode === 400){
      // modify the proxied response to produce a 200 OK 
      // the purpose of this proxy is to allow for custom error messages
      // sent via BuyerZone's web api
      // This proxy is only to be used when we cannot detect the XMLHttpRequest object
      // i.e. we are using IE 8 or 9
